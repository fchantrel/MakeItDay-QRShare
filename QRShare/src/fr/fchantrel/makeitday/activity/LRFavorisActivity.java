 ...
 *
 */
public class LRFavorisActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
    	...
    	String myIdQRShare = preferences.getString(CLE_ID_SHARE, "valeurDefaut");
	...
	private class JSONParse extends AsyncTask<String, String, List<InfosPro>> {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			pDialog = new ProgressDialog(LRFavorisActivity.this);
		...
		protected List<InfosPro> doInBackground(String... args) {
			...
			try {
				...
				String currentIdPro = "";
				// Getting JSON Object
				if((json != null) && (json.has("pjmyfavorites"))) {
					reponseJSONStorage = json.getJSONObject("pjmyfavorites");
					if(reponseJSONStorage.has("fav")) {
						JSONArray tabFavoris = reponseJSONStorage.getJSONArray("fav");
						for (int i = 0; i < tabFavoris.length(); i++) {
							JSONObject currentFavori = tabFavoris.getJSONObject(i);
							if(currentFavori.has("id")){
								currentIdPro = currentFavori.getString("id");
								InfosPro currentInfosPro = getInfosProJSON(currentIdPro);
								currentInfosPro.setNoImage(R.drawable.logo_pj);
								
								// on ajoute le pro courant à la liste
								retour.add(currentInfosPro);	
							}
						}
					}
				} // fin if pjmyfavorites
				else {
					retour.addAll(getMockedInfosPro());
				}
			} catch (Exception e){
				// on ajoute des donnes mockees pour la demo
		...
		protected void onPostExecute(List<InfosPro> lstInfoPro) {
			...
			for (Iterator<InfosPro> iterator = lstInfoPro.iterator(); iterator.hasNext();) {
        ...
        adapter=new LazyAdapter(LRFavorisActivity.this, songsList);
		...
		private InfosPro getInfosProXML(String currentIdPro) {
			...
			try{
				...
				String urlWSRecherche = URL_WS_REST_RECHERCHE.concat(currentIdPro);
		 ...
		 * @param currentIdPro
		 * @return
		 * Appel de l'API ws-rest-recherche en JSON
		 */
		private InfosPro getInfosProJSON(String currentIdPro) {
			...
			try{
				// construction de l'URL
				String urlWSRecherche = URL_WS_REST_RECHERCHE.concat(currentIdPro).concat(".json");
				...
				try {
					if(json.has("inscriptions")){
						JSONArray tabInscription = json.getJSONArray("inscriptions");
						if(tabInscription.length() > 0){
							JSONObject currentInscription = tabInscription.getJSONObject(0);
							String currentDenomination = currentInscription.getString("denomination");
							
							JSONObject myAdresse = currentInscription.getJSONObject("adresse");
							String currentAdresse = myAdresse.getString("libelle_adresse_complete");
							
							JSONArray tabMoyenComunication = currentInscription.getJSONArray("moyens_communication");
							JSONObject myMoyenComunication = tabMoyenComunication.getJSONObject(0);
							String currentTelephone = myMoyenComunication.getString("numero_formate");
